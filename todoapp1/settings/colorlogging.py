...
class RainbowLoggingHandler(logging.StreamHandler):
    ...
    _fmt = '[%(asctime)s] %(levelname)s %(pathname)s %(funcName)s:%(lineno)-4d [%(request_id)s] %(message)s'
    ...
    def format(self, record):
        ...
        """
        record.pathname = '%-16.16s' % record.pathname[-16:]
        record.levelname = '%-5.5s' % record.levelname
        record.funcName = '%-16.16s' % record.funcName
    ...
    def _encode(self, msg):
        ...
        if (2, 6, 0) <= sys.version_info < (3, 0, 0) and unicode and isinstance(msg, unicode):
            enc = getattr(self.stream, 'encoding') or 'utf-8'
